...
import { InvoicePDFValidationResult, ChargeLineItem, UsageLineItem } from '../types/invoice.types';
...
export interface PDFValidationOptions {
    ...
    usageDates?: string[];
    expectedCalling?: boolean;
    expectedSMS?: boolean;
    expectedData?: boolean;
}

export class SiebelBillingPDFPage {
    ...
    async validateInvoicePDF(options: PDFValidationOptions): Promise<InvoicePDFValidationResult> {
        ...
        const result: InvoicePDFValidationResult = {
            mobileNumber: '',
            planName: '',
            planRental: 0,
            billCycle: '',
        ...
        try {
            ...
            const fullScreenshot = await SiebelHelper.screenshot('POINT_1_2_3_Full_PDF_Header');
     ...
     */
    private async validatePoint1_BasicInfo(
        text: string, 
        ...
        const screenshot = await SiebelHelper.screenshot(`POINT1_MSISDN_Plan_${options.msisdn}`);
     ...
     */
    private async validatePoint2_BillSummary(
        text: string, 
        result: InvoicePDFValidationResult,
        screenshots: string[]
    ): Promise<void> {
        ...
        let match;
        const pastBills: { period: string; amount: number }[] = [];
        ...
        const screenshot = await SiebelHelper.screenshot('POINT2_Bill_Summary');
        screenshots.push(screenshot);
     ...
     */
    private async validatePoint3_AccountDetails(
        text: string, 
        result: InvoicePDFValidationResult,
        screenshots: string[]
    ): Promise<void> {
        ...
        const screenshot = await SiebelHelper.screenshot('POINT3_Account_Details');
        screenshots.push(screenshot);
     ...
     */
    private async validatePoint4_UsageDetails(
        text: string, 
        result: InvoicePDFValidationResult,
        screenshots: string[]
    ): Promise<void> {
        ...
        const screenshot = await SiebelHelper.screenshot('POINT4_Usage_Details');
        screenshots.push(screenshot);
     ...
     */
    private async validatePoint5_AllCharges(
        text: string, 
        result: InvoicePDFValidationResult,
        screenshots: string[]
    ): Promise<void> {
        ...
        const screenshot = await SiebelHelper.screenshot('POINT5_All_Charges_NonZero');
        screenshots.push(screenshot);

        console.log(`   💰 Non-zero charges found: ${result.allCharges.length}`);
        if (result.allCharges.length > 0) {
            console.log(`   🔝 Top 5 charges:`);
            result.allCharges.slice(0, 5).forEach(c => 
                console.log(`      - ${c.description}: ₹${c.charges}`)
            );
        }
    }
     ...
     */
    private async validatePoint6_UsageByDate(
        text: string,
        options: PDFValidationOptions,
        result: InvoicePDFValidationResult,
        screenshots: string[]
        ...
        const screenshot = await SiebelHelper.screenshot('POINT6_Usage_By_Date');
        screenshots.push(screenshot);
    ...
    private async extractPDFText(): Promise<string> {
        try {
            ...
            const pdfContent = await $('pre').getText();
     ...
     */
    private async extractPDFTextViaDownload(): Promise<string> {
        try {
