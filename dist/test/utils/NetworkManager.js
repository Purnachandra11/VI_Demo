...
class NetworkManager {
    // ── Public API ──────────────────────────────────────────────────────────────
    static async setNetworkType(deviceId, target, options = {}) {
        var _a, _b;
        const maxWait = (_a = options.maxWaitSeconds) !== null && _a !== void 0 ? _a : 35;
        const retries = (_b = options.retries) !== null && _b !== void 0 ? _b : 2;
     ...
     */
    static async getCurrentNetworkType(deviceId) {
        try {
            // ── Step 1: 5G NSA via TelephonyDisplayInfo ──────────────────────────
            // Only the most recent notifyDisplayInfoChanged entry matters
            const displayLog = await (0, adb_1.adbShell)(deviceId, `dumpsys telephony.registry | grep "notifyDisplayInfoChanged.*PhoneId=0" | tail -1`);
            ...
            const serviceState = await (0, adb_1.adbShell)(deviceId, `dumpsys telephony.registry | grep "mServiceState=" | head -1`);
            ...
            const csBlock = await (0, adb_1.adbShell)(deviceId, `dumpsys telephony.registry | grep "domain=CS.*accessNetworkTechnology" | head -1`);
            if (csBlock.includes('accessNetworkTechnology=NR'))
                return '5G';
            if (csBlock.includes('accessNetworkTechnology=LTE'))
                return '4G';
            if (csBlock.includes('accessNetworkTechnology=UMTS') ||
                csBlock.includes('accessNetworkTechnology=HSDPA') ||
                csBlock.includes('accessNetworkTechnology=HSPA'))
                return '3G';
            if (csBlock.includes('accessNetworkTechnology=GSM') ||
                csBlock.includes('accessNetworkTechnology=GPRS') ||
                csBlock.includes('accessNetworkTechnology=EDGE'))
                return '2G';
            return 'UNKNOWN';
        }
        catch {
     ...
     */
    static async isVoLTEEnabled(deviceId) {
        try {
            ...
            const vops = await (0, adb_1.adbShell)(deviceId, `dumpsys telephony.registry | grep "LteVopsSupportInfo" | head -5`);
            // Any cell currently supporting VoPS=3 means VoLTE is available
            if (vops.includes('mVopsSupport = 3'))
                return true;
            // Fallback to settings
            const setting = (await (0, adb_1.adbShell)(deviceId, `settings get global enhanced_4g_mode_enabled`)).trim();
            return setting === '1';
        }
        catch {
            return false;
     ...
     */
    static async forceReRegistration(deviceId) {
        console.log('    🔄  Re-registration via cmd connectivity…');
        // Try cmd connectivity first (works on Unisoc Android 12+)
        const enableOut = await (0, adb_1.adbShell)(deviceId, `cmd connectivity airplane-mode enable`);
        await delay(3000);
        const disableOut = await (0, adb_1.adbShell)(deviceId, `cmd connectivity airplane-mode disable`);
        ...
        await (0, adb_1.adbShell)(deviceId, `settings put global airplane_mode_on 1`);
        await delay(2000);
        await (0, adb_1.adbShell)(deviceId, `settings put global airplane_mode_on 0`);
        console.log('    ✓   airplane_mode setting toggled (no broadcast)');
    }
    /**
     * Wait for stable network.
     */
    static async waitForStableNetwork(deviceId, timeoutSeconds) {
    ...
    // ── Private ─────────────────────────────────────────────────────────────────
    static async applyNetworkMode(deviceId, target) {
        const mode = Network_types_1.NETWORK_MODE_MAP[target];
        // Method 1: settings database
        await (0, adb_1.adbShell)(deviceId, `settings put global preferred_network_mode ${mode}`);
        console.log(`    ✓   settings preferred_network_mode → ${mode}`);
        // Method 2: cmd phone (Android 12+ Unisoc)
        await (0, adb_1.adbShell)(deviceId, `cmd phone set-preferred-network-type-for-slot 0 ${mode}`);
    ...
    static async pollForNetwork(deviceId, target, maxWait) {
        let stable = 0, last = '';
        ...
        for (let s = 1; s <= maxWait; s++) {
            ...
            const cur = await this.getCurrentNetworkType(deviceId);
            if (s % 5 === 0)
                console.log(`         ${s}/${maxWait}s → ${cur}`);
            if (this.isAcceptable(target, cur)) {
                if (++stable >= 3)
                    return { matched: true, actual: cur };
            }
            else {
                if (cur !== last)
                    console.log(`         changed: ${last || 'N/A'} → ${cur}`);
                stable = 0;
                last = cur;
            }
        }
    ...
    static isAcceptable(target, actual) {
    ...
    static buildFailureReason(target, actual) {
        const t = target.toUpperCase(), a = actual.toUpperCase();
        if (t === '3G' && (a === '4G' || a === '5G'))
