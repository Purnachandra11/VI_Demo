...
const SIEBEL_PROJECT_ROOT = path.resolve(
  __dirname,
  "..",
  "siebel-crm-automation",
);
// require('./config.js')
...
app.post("/api/search-files", (req, res) => {
  ...
  fs.readdir(FILES_DIR, (err, files) => {
    ...
    if (matchedFiles.length === 0) {
      console.log(
        `No files found for today's date, showing all matching files`,
      );
    ...
    res.json({
      ...
      message:
        matchedFiles.length === finalFiles.length
...
app.get("/api/admin/users", async (req, res) => {
  try {
    const users = await Registration.find({ is_deleted: 0 }).select(
      "name email created",
    );
...
app.get("/api/admin/reports", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to load reports." });
  }
});
...
app.delete("/api/admin/reports", async (req, res) => {
  try {
    ...
    if (!name) {
      return res
        .status(400)
        .json({ success: false, message: "Report name is required." });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to delete report." });
  }
});
...
function checkNetworkMatch(current, target) {
  ...
  if (target === "3G")
    return (
      current.includes("HSPA") ||
      current.includes("UMTS") ||
      current.includes("3G")
    );
  if (target === "2G")
    return (
      current.includes("EDGE") ||
      current.includes("GPRS") ||
      current.includes("2G")
    );
...
async function getBrowserPage() {
  ...
  if (!pageInstance || pageInstance.isClosed()) {
    ...
    await pageInstance.goto(VI_URL, {
      waitUntil: "domcontentloaded",
      timeout: 30000,
    });

    // NEW: let the SPA fully hydrate before any real fill happens.
    // This is what was causing the first bulk item to abort —
    // its fill was racing the page's hydration/mount cycle.
    await pageInstance
      .waitForLoadState("networkidle", { timeout: 15000 })
      .catch(() => {});
    await pageInstance
      .locator(SELECTORS.mobileInput)
      .waitFor({ state: "visible", timeout: 15000 })
      .catch(() => {});

    // Warm-up: type + clear a throwaway value to flush any
    // hydration-driven input resets before real validations begin.
    try {
      const warmupInput = pageInstance.locator(SELECTORS.mobileInput);
      await warmupInput.click({ clickCount: 3 });
      await warmupInput.fill("9999999999");
      await pageInstance.waitForTimeout(800);
      await warmupInput.click({ clickCount: 3 });
      await warmupInput.fill("");
      await pageInstance.waitForTimeout(300);
      console.log("[Warmup] Page hydration settled, input warmed up.");
    } catch (warmupErr) {
      console.warn(
        "[Warmup] Warmup step failed (non-fatal):",
        warmupErr.message,
      );
    }
  }
...
async function extractCircleFromPage(page) {
  try {
    ...
    const sortedNames = [...CIRCLE_NAME_TO_CODE.keys()].sort(
    ...
    console.warn(
      `[CircleCheck] ⚠️ Could not map circle text to a known code: "${rawText}"`,
    );
    return { rawText, code: "UNKNOWN" };
  } catch (err) {
    console.warn(
      "[CircleCheck] Error extracting circle text:",
...
async function validateOnPage(page, mobileNumber, bypassCache = false) {
  console.log(
    `[Validate] Starting validation for ${mobileNumber} (bypassCache: ${bypassCache})`,
  );
  ...
  if (typedValue !== mobileNumber) {
    console.warn(
      `[Validate] Fill mismatch for ${mobileNumber} (got "${typedValue}") — retrying once`,
    );
    await input.click({ clickCount: 3 });
    await input.fill(mobileNumber);
    ...
    if (retryValue !== mobileNumber) {
      console.warn(
        `[Validate] Fill still mismatched for ${mobileNumber} after retry (got "${retryValue}")`,
      );
    }
  }
  ...
  if (oldCircle) {
    try {
      console.log(
        `[Validate] Waiting for circle to change from "${oldCircle}" for ${mobileNumber}`,
      );
      await page.waitForFunction(
        (oldCircleText) => {
          const el = document.querySelector(
            ".layoutCSS_locationContainer__7HdTf p",
          );
          if (!el) return false;
      ...
      console.warn(
        `[Validate] Circle text didn't change within timeout for ${mobileNumber}`,
      );
    }
  } else {
    try {
      console.log(
        `[Validate] Waiting for circle text to appear for ${mobileNumber}`,
      );
  ...
  if (finalValue !== mobileNumber) {
    console.warn(
      `[Validate] Input no longer shows ${mobileNumber} (shows "${finalValue}") — aborting this validation`,
    );
    const result = {
      number: mobileNumber,
      isValid: false,
      message:
        "Validation aborted: input value changed mid-flight (concurrent request collision)",
      actualCircleText: "",
      actualCircleCode: "UNKNOWN",
  ...
  const errorText = hasError
    ? (await errorLocator.first().textContent()).trim()
    : "";
...
app.post("/api/validate", async (req, res) => {
  ...
  console.log(
    `[API] /api/validate called with number: ${number}, bypassCache: ${bypassCache}`,
  );
  ...
  try {
    ...
    const result = await withPageLock(() =>
      validateOnPage(page, number, bypassCache),
    );
...
app.post("/api/validate/bulk", async (req, res) => {
  ...
  console.log(
    `[API] /api/validate/bulk called with ${rawNumbers?.length || 0} numbers, bypassCache: ${bypassCache}`,
  );
  ...
  if (!Array.isArray(rawNumbers) || rawNumbers.length === 0) {
    return res
      ...
      .json({
        error: "Invalid input",
        detail: '"numbers" must be a non-empty array',
      });
  }
  ...
  try {
    ...
    for (let i = 0; i < validEntries.length; i++) {
      const entry = validEntries[i];
      console.log(
        `[API] Processing bulk item ${i + 1}/${validEntries.length}: ${entry.number}`,
      );

      try {
        let result = await withPageLock(() =>
          validateOnPage(page, entry.number, bypassCache),
        );

        // NEW: if it aborted due to the mid-flight collision, retry once more —
        // by now the page should be fully settled.
        if (!result.isValid && result.message?.includes("mid-flight")) {
          console.warn(
            `[API] ${entry.number} hit mid-flight collision — retrying once`,
          );
          await page.waitForTimeout(500);
          result = await withPageLock(() =>
            validateOnPage(page, entry.number, true),
          );
        }
    ...
    const circleMismatchCount = results.filter(
      (r) => r.circleMatched === false,
    ).length;
    ...
    console.log(
      `[API] Bulk response: ${validCount} valid, ${invalidProcessed} invalid, ${circleMismatchCount} mismatches`,
    );
    res.json(response);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
  ...
  res.json({
    ...
    average_age_ms:
      validationCache.size > 0
        ? Math.round(totalAge / validationCache.size)
        : 0,
...
app.post("/api/process-validation-and-send-emails", async (req, res) => {
  try {
    ...
    const emailRecipients =
      recipients.length > 0 ? recipients : ["kalidindi.chandra@qdegrees.org"];
    ...
    console.log(
      `📊 Processing: ${validNumbers.length} valid, ${invalidNumbers.length} invalid`,
    );
      ...
      checkPort(4723, (isListening) => {
        if (isListening) {
          ...
          res.json({
            success: false,
            message:
              "Appium failed to start - port 4723 not listening after 20 seconds",
          });
        }
      });
    }, attempt * 2000);
    ...
    exec("netstat -ano | findstr :4723", (error, stdout) => {
      ...
      exec('taskkill /F /IM node.exe /FI "WINDOWTITLE eq appium*"', () => {
        setTimeout(() => {
          appiumStatus = "stopped";
          console.log(
            ` Appium server stopped. Killed ${killedProcesses} process(es)`,
          );
          res.json({
            success: true,
            message: `Appium server stopped. Killed ${killedProcesses} process(es)`,
...
app.get("/api/servers/status", (req, res) => {
  ...
  exec("adb devices", (adbError, adbStdout) => {
    ...
    if (isWindows) {
      ...
      exec("netstat -ano | findstr :4723", (appiumError, appiumStdout) => {
        const appiumRunning =
          appiumStdout && appiumStdout.includes("LISTENING");
        appiumStatus = appiumRunning ? "running" : "stopped";
      ...
      exec("lsof -i :4723 -t", (appiumError, appiumStdout) => {
        const appiumRunning =
          !appiumError && appiumStdout && appiumStdout.trim().length > 0;
        appiumStatus = appiumRunning ? "running" : "stopped";
            ...
            async (err2, version) => {
              ...
              try {
                ...
                if (ussd && ussd.success && ussd.phoneNumber) {
                  ...
                  ussdSuccess = true;
                  console.log(
                    `USB device connected with number: ${ussd.phoneNumber}`,
                  );
                } else {
                  console.log(
                    `No phone number detected via USSD for ${deviceId}`,
                  );
                  if (ussd && ussd.error) {
                ...
                console.error(
                  `USSD exception for USB device ${deviceId}:`,
                  ussdError.message,
                );
              }
  ...
  try {
    ...
    if (!connectResult.success || !connectResult.output.includes("connected")) {
      return res.json({
        success: false,
        message:
          "Paired successfully but connection failed. Try connecting again.",
      });
    }
    ...
    const device = {
      ...
      androidVersion: versionResult.success
    ...
    try {
      ...
      if (ussd && ussd.success && ussd.phoneNumber) {
        ...
        ussdSuccess = true;
        console.log(
          `✅ Wireless device connected with number: ${ussd.phoneNumber}`,
        );
      } else {
      ...
      console.error(
        `❌ USSD exception for wireless device ${deviceId}:`,
        ussdError.message,
      );
    }
  ...
  try {
    ...
    try {
      ...
      if (ussd && ussd.success && ussd.phoneNumber) {
        ...
        ussdSuccess = true;
        console.log(
          `✅ Existing wireless device connected with number: ${ussd.phoneNumber}`,
        );
      } else {
        console.log(`⚠️ No phone number detected via USSD for ${deviceId}`);
        if (ussd && ussd.error) {
      ...
      console.error(
        `❌ USSD exception for existing wireless device ${deviceId}:`,
        ussdError.message,
      );
    }
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const match = cleaned.match(
        /^([^\s]+)\s+(device|connected|unauthorized|offline)$/i,
      );
  ...
  try {
    ...
    for (const line of lines) {
      const match = line.match(
        /^([^\s\t]+)[\s\t]+(device|connected|unauthorized|offline)$/,
      );
    ...
    if (existingDevice && existingDevice.phoneNumber) {
      ...
      console.log(
        `[Device] ✅ Device ${deviceId} already has SIM: ${device.phoneNumber}`,
      );
    } else {
      ...
      try {
        ...
        if (ussd && ussd.success && ussd.phoneNumber) {
          ...
          ussdSuccess = true;
          console.log(
            `✅ Device ${deviceId} connected with number: ${ussd.phoneNumber}`,
          );
        } else {
          console.log(`⚠️ No phone number detected via USSD for ${deviceId}`);
          if (ussd && ussd.error) {
        ...
        console.error(
          `❌ USSD exception for device ${deviceId}:`,
          ussdError.message,
        );
      }
    }
    ...
    (error, stdout) => {
      ...
      res.json({
        ...
        message: available
          ...
          : `${networkType} not available at your location`,
      });
    },
  );
});
...
app.post("/api/network/status", (req, res) => {
  ...
  exec(
    `adb -s "${deviceId}" shell getprop gsm.network.type`,
    (error, networkStdout) => {
      const networkType = networkStdout
        ? networkStdout.trim().toLowerCase()
        : "unknown";

      // Get VoLTE status (simplified - you might need a different approach)
      exec(
        `adb -s "${deviceId}" shell dumpsys telephony.registry | grep mImsState`,
        (volteError, volteStdout) => {
...
app.get("/api/reports/manifest", (req, res) => {
  ...
  if (fs.existsSync(summaryPath)) {
    try {
      payload.executionSummary = JSON.parse(
...
app.get("/api/reports/execution-summary.html", (req, res) => {
  ...
  if (!fs.existsSync(summaryHtml)) {
    return res
      ...
      .json({
        success: false,
        message: "No execution summary yet. Run tests first.",
      });
  }
...
function broadcastToDevice(deviceId, progressData) {
  ...
  console.log(
    `📡 Broadcast to device ${deviceId}:`,
    progressData.type || "update",
  );
}
    ...
    onClose: (code) => {
      ...
      if (!responded) {
        ...
        if (code === 0) {
          res
            .status(200)
            .json({ status: "success", exitCode: code, stdout, stderr });
        } else {
          res
            .status(500)
            .json({ status: "failed", exitCode: code, stdout, stderr });
        }
      }
    },
    onError: (error) => {
...
const swiftReportsDir = path.join(
  __dirname,
  "..",
  "swift-crm-automation",
  "reports",
);
const swiftFinalReportsDir = path.join(
  __dirname,
  "..",
  "swift-crm-automation",
  "finalreports",
...
app.post("/api/swift/captcha-response", (req, res) => {
  try {
    ...
    console.log(
      `[SWIFT] CAPTCHA response received: ${answer} for user ${username}`,
    );
    res.json({ success: true });
  } catch (e) {
...
app.post("/api/swift/upload", swiftUpload.single("excel"), (req, res) => {
  ...
  const sessionId =
    requestedSessionId || crypto.randomBytes(16).toString("hex");
  ...
  if (req.body && req.body.matchedRows) {
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        ...
        console.log(
          `[SWIFT] 📋 Received ${matchedRows.length} frontend-validated matched row(s) for session ${sessionId}`,
        );
      }
    } catch (e) {
      console.warn(
        `[SWIFT] ⚠️ Could not parse matchedRows from upload: ${e.message}`,
      );
    }
  }
  if (!matchedRows) {
    console.warn(
      "[SWIFT] ⚠️ No matchedRows received with upload — orchestrator will fall back to SWIFT=Yes Excel filtering.",
    );
  }

  console.log(
    `[SWIFT] 📄 File uploaded: ${req.file.originalname} (Session: ${sessionId})`,
  );
...
app.use(
  "/screenshots",
  express.static(
    path.join(__dirname, "..", "swift-crm-automation", "screenshots"),
  ),
);
...
app.get("/api/swift/report/files", (req, res) => {
  ...
  files.sort(
    (a, b) =>
      fs.statSync(b.path).mtime.getTime() - fs.statSync(a.path).mtime.getTime(),
  );
  res.json({
    success: true,
...
app.get("/api/swift/download-report", (req, res) => {
  if (!latestReportPath || !fs.existsSync(latestReportPath)) {
    ...
    if (fs.existsSync(swiftFinalReportsDir)) {
      candidates.push(
        ...fs
          .readdirSync(swiftFinalReportsDir)
          .filter((name) => name.endsWith(".zip"))
          .map((name) => path.join(swiftFinalReportsDir, name)),
      );
    }
    if (fs.existsSync(swiftReportsDir)) {
      candidates.push(
        ...fs
          .readdirSync(swiftReportsDir)
          .filter((name) => name.endsWith(".xlsx"))
          .map((name) => path.join(swiftReportsDir, name)),
      );
      candidates.push(
        ...fs
          .readdirSync(swiftReportsDir)
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `WebSocket server ready on ws://localhost:${PORT} and ws://${SERVER_IP}:${PORT}`,
  );
