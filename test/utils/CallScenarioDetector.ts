...
function containsIgnoreCase(source: string | null | undefined, ...keywords: string[]): boolean {
 ...
 */
export function detectCallScenario(
  pageSource: string,
  ...
  if (
    containsIgnoreCase(pageSource, 'network') &&
  ...
  if (
    containsIgnoreCase(pageSource, 'not reachable', 'out of coverage', 'unreachable', 'switched off')
  ) {
    return {
      scenarioNumber: 7,
  ...
  if (
    containsIgnoreCase(pageSource, 'outgoing') &&
  ...
  if (
    containsIgnoreCase(pageSource, 'incoming') &&
    containsIgnoreCase(pageSource, 'barred', 'restricted')
  ) {
    return {
      scenarioNumber: 9,
  ...
  if (
    containsIgnoreCase(pageSource, 'crbt', 'caller tune', 'ring back tone') &&
    callConnected
  ) {
    return {
      scenarioNumber: 12,
...
export function getScenarioEmoji(scenarioNumber: number): string {
