...
try {
  ...
  const testDir = path.join(__dirname, '..', '..');   // two levels up from test/specs/
  if (fs.existsSync(testDir)) {
    ...
    if (fs.existsSync(testSubDir)) {
      ...
      const entries = fs.readdirSync(testSubDir);

      // TS7006 fix: explicitly type the `f` parameter as `string`
      const subdirs = entries.filter((f: string) =>
        fs.statSync(path.join(testSubDir, f)).isDirectory()
      );
