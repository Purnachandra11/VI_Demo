...
export class RechargePage {
  ...
  async runPreTestVerification(msisdn: string): Promise<{
    ...
    try {
      ...
      if (voiceUsage?.length > 0) failedCategories.push(`Voice (${voiceUsage.length})`);
      if (dataUsage?.length > 0) failedCategories.push(`Data (${dataUsage.length})`);
      if (smsUsage?.length > 0) failedCategories.push(`SMS (${smsUsage.length})`);

      if (failedCategories.length > 0) {
        result.success = false;
        result.reason = `PreTest Failed: Active usage detected in ${failedCategories.join(", ")} - MSISDN Number Not Clean`;
  ...
  /**
   * Dedicated click helper for PreTest only.
   * This does NOT interfere with the existing clickAndWait() used in parseActiveOffers().
   */
