...
exports.EnhancedExcelReader = exports.ExcelReader = void 0;
exports.readCallingRows = readCallingRows;
exports.readSmsRows = readSmsRows;
exports.readDataUsageRows = readDataUsageRows;
exports.readSIMLatchRows = readSIMLatchRows;
...
function cellStr(v) {
    ...
    if (typeof v === 'object' && 'result' in v) {
        return String((_a = v.result) !== null && _a !== void 0 ? _a : '');
    }
...
function getHeaderColumns(sheet) {
    ...
    headerRow.eachCell({ includeEmpty: false }, (cell, colNumber) => {
        ...
        if (label)
            cols.set(label, colNumber);
    });
    return cols;
}
function matchColumn(headers, needles) {
...
function pickDurationFromRow(row, colDuration) {
    if (colDuration <= 0)
        return 15;
...
function normalizeNetworkType(network) {
    ...
    if (upper.includes('5G'))
        return '5G';
    if (upper.includes('4G') || upper.includes('LTE'))
        return '4G';
    if (upper.includes('3G'))
        return '3G';
    if (upper.includes('2G'))
        return '2G';
    if (upper.includes('AUTO'))
        return 'AUTO';
    return 'AUTO';
}
function normalizeCallType(type) {
    ...
    if (upper.includes('VIDEO'))
        return 'VIDEO';
    if (upper.includes('CONFERENCE') || upper.includes('3-WAY'))
        return 'CONFERENCE';
    if (upper.includes('VOLTE'))
        return 'VOLTE';
    return 'VOICE';
}
function normalizeDirection(direction) {
    ...
    if (upper.includes('INCOMING') || upper.includes('INBOUND') || upper === '←')
        return 'INCOMING';
    return 'OUTGOING';
}
...
function parseDoubleValue(value, defaultValue) {
    if (!value || value.trim() === '')
        return defaultValue;
    try {
        const cleaned = value.replace(/[^0-9.-]/g, '');
...
function logCallingTestCase(testCase) {
    ...
    console.log(`${emoji} ${direction} ${testCase.name.padEnd(40)} | ${testCase.aPartyNumber || 'N/A'} -> ${testCase.bPartyNumber} | ${testCase.preferredNetwork} | ${testCase.duration}s | ${testCase.attempts} attempts${volte}${conference}`);
}
function logSMSTestCase(testCase) {
...
function printSMSTestSummary(testCases) {
...
function printCallingTestSummary(testCases) {
...
async function readCallingRows(filePath) {
    ...
    const sheet = wb.getWorksheet('Calling') ||
    ...
    if (colB <= 0) {
        throw new Error(`Could not find B Party column in sheet "${sheet.name}". Headers: ${[...headers.keys()].join(', ')}`);
    }
    const rows = [];
...
async function readSmsRows(filePath) {
    const resolved = path_1.default.resolve(filePath);
    if (!fs_1.default.existsSync(resolved)) {
...
async function readDataUsageRows(filePath, sheetName = 'DataUsage') {
    const resolved = path_1.default.resolve(filePath);
    if (!fs_1.default.existsSync(resolved)) {
        console.log('❌ File path cannot be null or empty');
        return [];
...
async function readSIMLatchRows(filePath) {
    const resolved = path_1.default.resolve(filePath);
    if (!fs_1.default.existsSync(resolved)) {
        console.log('❌ File path cannot be null or empty');
        return [];
    ...
    sheet.eachRow((row, rowNumber) => {
        ...
        if (colPartyNumber > 0) {
            ...
            if (value && typeof value === 'object' && 'result' in value) {
                partyNumber = String((_a = value.result) !== null && _a !== void 0 ? _a : '');
            }
            else if (value && typeof value === 'object' && 'text' in value) {
                partyNumber = value.text || '';
            }
            else if (value) {
...
class ExcelReader {
    ...
    async getTestPlanData(sheetName = 'Testplan') {
    ...
    async getPlanDetails(sheetName = 'Plan details') {
        const worksheet = this.workbook.getWorksheet(sheetName);
        if (!worksheet) {
            throw new Error(`Sheet '${sheetName}' not found`);
        }
...
exports.EnhancedExcelReader = EnhancedExcelReader;
EnhancedExcelReader.readCallingTestData = readCallingRows;
EnhancedExcelReader.readSMSTestData = readSmsRows;
EnhancedExcelReader.readDataUsageTestData = readDataUsageRows;
EnhancedExcelReader.readSIMAutoLatchTestData = readSIMLatchRows;
