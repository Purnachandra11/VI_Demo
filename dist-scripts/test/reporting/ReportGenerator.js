...
class ReportGenerator {
    ...
    // CALLING REPORT 
    static async generateCallingExcelReport(results) {
    ...
    static async generateCallingHTMLReport(results) {
        const dialingNumber = process.env.aPartyNumber || '';
...
<div class='container'>
    ...
    // ========== SMS REPORT METHODS ==========
    static async generateSMSExcelReport(results) {
        const dialingNumber = process.env.aPartyNumber || '';
    ...
    static async generateSMSTestReport(results, deviceId, deviceNumber) {
        const dialingNumber = process.env.aPartyNumber || '';
...
<div class='container'>
    ...
    static async generateDataUsageExcelReport(results) {
    ...
    static async generateDataUsageHTMLReport(results) {
        const dialingNumber = process.env.DaPartyNumber || process.env.aPartyNumber || '';
...
<div class='container'>
    ...
    static async generateSIMAutoLatchExcelReport(results) {
        const dialingNumber = process.env.aPartyNumber || '';
    ...
    static async generateSIMAutoLatchHTMLReport(results) {
        const dialingNumber = process.env.aPartyNumber || '';
...
<div class='container'>
    ...
    static generateEnhancedSummarySection(results) {
        if (results.length === 0) {
...
<div class='summary'>
    ...
    static generateSummarySection(results) {
        const total = results.length;
        const success = results.filter(r => r.finalStatus === 'SUCCESS').length;
...
<div class='summary'>
    ...
    static generateEnhancedCallingTable(results) {
    ...
    static generateEnhancedSMSSummarySection(results, deviceId, deviceNumber) {
        if (results.length === 0) {
            return "<div class='summary'><p>No test results available</p></div>";
        }
        const total = results.length;
...
<div class='summary'>
    ...
    static generateEnhancedSMSTable(results) {
        let html = `<table><thead><tr>
    ...
    static generateDataUsageTable(results) {
        let html = `<table><thead><tr>
    ...
    static generateSIMAutoLatchSummarySection(results) {
        if (results.length === 0) {
            return "<div class='summary'><p>No test results available</p></div>";
        }
        const total = results.length;
...
<div class='summary'>
    ...
    static generateSIMAutoLatchTable(results) {
        let html = `<table><thead><tr>
        ...
        for (const result of results) {
            ...
            if (timeMs > 0) {
                if (timeMs <= 30000)
                    timeStatus = '<small>✓ Fast (<30s)</small>';
                else if (timeMs <= 60000)
                    timeStatus = '<small>⚠ Acceptable</small>';
                else
                    timeStatus = '<small>✗ Slow (>60s)</small>';
            }
    ...
    static getSIMAutoLatchStatusClass(status) {
        if (status === 'PASS')
            return 'success';
        if (status === 'MARGINAL')
            return 'partial';
        if (status === 'SLOW' || status === 'FAIL' || status === 'ERROR')
            return 'failed';
        return '';
    }
    static getVerificationStatusDisplay(status) {
        switch (status) {
    ...
    static getDeliveryStatusDisplay(status) {
        switch (status) {
            case 'SUCCESS':
    ...
    static cleanNetworkType(networkType) {
...
_a = ReportGenerator;
ReportGenerator.REPORT_DIR = 'test-output/comprehensive-reports/';
(() => {
    if (!fs.existsSync(_a.REPORT_DIR)) {
        fs.mkdirSync(_a.REPORT_DIR, { recursive: true });
    }
})();
