...
class ImprovedMessagingPage {
    constructor(driver, deviceId) {
        this.driver = driver;
        this.deviceId = deviceId;
        this.verifier = new MessageVerifier_1.MessageVerifier(driver);
        this.voiceHandler = new VoiceMessageHandler_1.VoiceMessageHandler(driver);
    }
    /**
     * INDIVIDUAL SMS (Text Message)
     */
    async sendIndividualSMS(phoneNumber, message) {
        try {
            console.log(` Starting Individual SMS to: ${phoneNumber}`);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'STARTED', 'Starting SMS send', 0);
            await this.openMessagingApp();
            await this.ensureMainScreen();
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'APP_OPENED', 'Messaging app ready', 20);
            await this.startNewConversation();
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'NEW_CONVERSATION', 'Conversation started', 40);
            await this.enterPhoneNumber(phoneNumber);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'NUMBER_ENTERED', 'Phone number entered', 60);
            await this.enterMessage(message);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'MESSAGE_ENTERED', 'Message typed', 80);
            await this.sendMessage();
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'SENDING', 'Sending message', 90);
            ...
            if (sent) {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'COMPLETED', 'SMS sent successfully', 100);
            }
            else {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'FAILED', 'SMS verification failed', 0);
            }
            ...
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'ERROR', `Exception: ${error}`, 0);
            return false;
        }
    }
    /**
     ...
     */
    async sendIndividualSMSWithTimestamp(phoneNumber, message) {
        try {
            console.log(` Starting Individual SMS to: ${phoneNumber}`);
            this.verifier.setRecipientNumber(phoneNumber);
     ...
     */
    async sendIndividualVoiceMessage(phoneNumber) {
        return this.voiceHandler.sendVoiceMessage(phoneNumber);
    }
    /**
     * INDIVIDUAL VOICE MESSAGE FIXED with verification
     */
    async sendIndividualVoiceMessageFixed(phoneNumber) {
        try {
            console.log(`🎤 Starting Individual Voice Message to: ${phoneNumber}`);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'STARTED', 'Starting voice message', 0);
            // Set recipient number for verification
            this.verifier.setRecipientNumber(phoneNumber);
            const result = await this.voiceHandler.sendVoiceMessageWithDetails(phoneNumber);
            const voiceSent = result.get('success');
            if (voiceSent) {
                // Get detailed verification with timestamp
                const sentInfo = await this.verifier.getLatestSentMessage();
                if (sentInfo) {
                    await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'COMPLETED', `Voice message sent at: ${sentInfo.getFormattedDate()}`, 100);
                    console.log(` Voice message sent at: ${sentInfo.getFormattedDate()}`);
                    return true;
                }
                else {
                    await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'VERIFICATION_FAILED', 'Voice message verification failed', 50);
                    return false;
                }
            }
            else {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, phoneNumber, 'FAILED', 'Voice recording failed', 0);
                return false;
            }
        }
        catch (error) {
     ...
     */
    async sendGroupSMS(groupName, message) {
        const result = { success: false };
        try {
            console.log(`👥 Starting Group SMS to: ${groupName}`);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'STARTED', 'Starting group SMS', 0);
            await this.openMessagingApp();
            await this.ensureMainScreen();
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'APP_OPENED', 'Messaging app ready', 20);
            const groupOpened = await this.quickSearchGroup(groupName);
            if (!groupOpened) {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'GROUP_NOT_FOUND', `Group not found: ${groupName}`, 0);
                result.error = `Group not found: ${groupName}`;
                return result;
            }
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'GROUP_OPENED', 'Group conversation opened', 50);
            // FIX: Explicitly type the variable and ensure await is used
            const participantCount = await this.countGroupParticipants();
            // Now participantCount is definitely a number
            console.log(`📊 Group participants count: ${participantCount}`);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'COUNTING', `Participants counted: ${participantCount}`, 60);
            await this.enterMessage(message);
            await this.sendMessage();
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'MESSAGE_ENTERED', 'Message typed', 80);
            // Use unified verification
            const sent = await this.verifier.verifyMessageSent(message);
            if (sent) {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'COMPLETED', `Group SMS sent successfully to ${participantCount} participants`, 100);
            }
            else {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'VERIFICATION_FAILED', 'Group SMS verification failed', 0);
            }
            result.success = sent;
            ...
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'ERROR', `Exception: ${error}`, 0);
     ...
     * GROUP VOICE MESSAGE
     */
    async sendGroupVoiceMessage(groupName) {
        const result = { success: false };
        try {
            console.log(`🎤 Starting Group Voice Message to: ${groupName}`);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'STARTED', 'Starting group voice message', 0);
            await this.openMessagingApp();
            await this.ensureMainScreen();
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'APP_OPENED', 'Messaging app ready', 20);
            ...
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'GROUP_OPENED', 'Group conversation opened', 40);
            // FIX: Explicitly type the variable and ensure await is used
            const participantCount = await this.countGroupParticipants();
            // Now participantCount is definitely a number
            console.log(`📊 Group participants count: ${participantCount}`);
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'COUNTING', `Participants counted: ${participantCount}`, 50);
            console.log('  🎤 Holding audio button for 5 seconds...');
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'RECORDING', 'Recording voice message', 60);
            const audioHeld = await this.holdAudioButtonRobust(5000);
            if (!audioHeld) {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'RECORDING_FAILED', 'Failed to hold audio button', 0);
                result.error = 'Failed to hold audio button';
                return result;
            }
            console.log('  ⏱️ Waiting 2 seconds for processing...');
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'PROCESSING', 'Processing recording', 70);
            await this.driver.pause(2000);
            console.log('  📤 Sending voice message...');
            await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'SENDING', 'Sending voice message', 80);
            let sent = await this.sendVoiceMessageRobust();
            if (sent) {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'SENT', 'Voice message sent, verifying', 90);
                // Use unified verification for voice message
                sent = await this.verifier.verifyMessageSent('');
                if (sent) {
                    await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'COMPLETED', `Group voice message sent to ${participantCount} participants`, 100);
                }
                else {
                    await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'VERIFICATION_FAILED', 'Voice message verification failed', 0);
                }
            }
            else {
                await (0, progressReporter_1.reportSMSProgress)(this.deviceId, groupName, 'SEND_FAILED', 'Failed to send voice message', 0);
            }
            result.success = sent;
            result.participantCount = participantCount;
    ...
    async holdAudioButtonRobust(milliseconds) {
        try {
    ...
    async findAudioButtonWithRetry() {
        const locators = [
    ...
    // ==================== GROUP METHODS ====================
    async quickSearchGroup(groupName) {
        try {
    ...
    async clickVisibleGroup(groupName) {
        try {
    ...
    async searchAndOpenGroup(groupName) {
        try {
    ...
    async findAndClickGroupInSearchResults(groupName) {
        try {
    ...
    async findGroupByScrolling(groupName) {
        try {
...
exports.ImprovedMessagingPage = ImprovedMessagingPage;
/** MessagingPage wrapper for backward compatibility */
class MessagingPage extends ImprovedMessagingPage {
    constructor(driver, deviceId) {
        super(driver, deviceId || String(driver.capabilities['appium:udid'] || 'device'));
    }
}
exports.MessagingPage = MessagingPage;
