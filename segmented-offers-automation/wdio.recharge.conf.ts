import * as dotenv from 'dotenv';
import * as fs from 'fs';
import * as path from 'path';
dotenv.config();

const HEADLESS = process.env.HEADLESS !== 'false';

const chromeArgs = [
  '--disable-blink-features=AutomationControlled',
  '--ignore-certificate-errors',
  '--ignore-ssl-errors',
  '--allow-insecure-localhost',
  '--window-size=1920,1080',
  '--disable-web-security',
  '--disable-features=IsolateOrigins,site-per-process',
  '--no-sandbox',
  '--disable-dev-shm-usage',
  '--disable-gpu',
  '--disable-software-rasterizer',
  '--disable-extensions',
  '--disable-setuid-sandbox',
  '--disable-browser-side-navigation',
  '--disable-infobars',
  '--disable-notifications',
  '--disable-features=OutOfBlinkCors',
  '--disable-features=BlockInsecurePrivateNetworkRequests',
  '--disable-features=SameSiteByDefaultCookies,CookiesWithoutSameSiteMustBeSecure',
  '--force-fieldtrials=',
  '--use-system-default-privacy-screen',
  '--dns-prefetch-disable',
];

if (HEADLESS) {
  chromeArgs.push('--headless=new');
} else {
  chromeArgs.push('--start-maximized');
}

export const config = {
  runner: 'local',
  specs: ['./src/specs/swift_recharge_spec.ts'],
  exclude: [],
  maxInstances: 1,
  capabilities: [
    {
      browserName: 'chrome',
      browserVersion: process.env.CHROME_VERSION || '149',
      'goog:chromeOptions': {
        args: chromeArgs,
        excludeSwitches: ['enable-automation'],
      },
      // 🔥 ADDED: Logging preferences for debugging
      'goog:loggingPrefs': {
        browser: 'ALL',
        driver: 'ALL',
        performance: 'ALL',
      },
    },
  ],
  framework: 'mocha',
  mochaOpts: {
    ui: 'bdd',
    timeout: 24 * 60 * 60 * 1000,
  },
  autoCompileOpts: {
    autoCompile: true,
    tsNodeOpts: {
      transpileOnly: true,
      project: './tsconfig.json',
    },
  },
  reporters: [
    'spec',
    [
      'allure',
      {
        outputDir: 'allure-results',
        disableWebdriverStepsReporting: false,
        disableWebdriverScreenshotsReporting: false,
      },
    ],
  ],
  
  // 🔥 CHANGED: From 'silent' to 'info' for debugging
  logLevel: 'info',  // Changed from 'silent'
  
  bail: 0,
  baseUrl: 'https://swiftcrm.vodafoneidea.in',
  
  // 🔥 INCREASED: Timeouts
  waitforTimeout: 30000,
  connectionRetryTimeout: 180000,  
  connectionRetryCount: 5,         // Increased from 3

  before: async () => {
    if (!HEADLESS) {
      await browser.maximizeWindow();
    }

    const screenshotsDir = path.resolve('./screenshots');
    if (!fs.existsSync(screenshotsDir)) {
      fs.mkdirSync(screenshotsDir, { recursive: true });
    }

    const captchaDir = path.resolve('./captcha_screenshots');
    if (!fs.existsSync(captchaDir)) {
      fs.mkdirSync(captchaDir, { recursive: true });
    }

    const reportsDir = path.resolve('./reports');
    if (!fs.existsSync(reportsDir)) {
      fs.mkdirSync(reportsDir, { recursive: true });
    }

    console.log(`[WDIO] Single-batch run starting — Headless: ${HEADLESS}`);
  },

  afterTest: async (test, _context, { error }) => {
    if (error) {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const screenshotPath = path.resolve(`./screenshots/FAILED_${test.title.replace(/\s+/g, '_')}_${timestamp}.png`);
      try {
        await browser.saveScreenshot(screenshotPath);
        console.error(`[Hook] Screenshot saved for failed test: ${test.title}`);
      } catch (e) {
        console.warn('[Hook] Could not save screenshot:', e);
      }
    }
  },

  // 🔥 FIXED: Added delay to allow cleanup
  onComplete: async () => {
    console.log('[WDIO] Batch finished — browser session can be closed manually.');
    // Wait 3 seconds to allow any pending commands to complete
    await new Promise(resolve => setTimeout(resolve, 3000));
    console.log('[WDIO] Cleanup complete — exiting.');
  },

  // 🔥 ADDED: After session hook
  afterSession: async () => {
    console.log('[WDIO] Session ending — ensuring clean shutdown...');
    try {
      await browser.pause(1000);
    } catch (_) {
      // Browser may already be closed
    }
    console.log('[WDIO] Session ended.');
  },
};