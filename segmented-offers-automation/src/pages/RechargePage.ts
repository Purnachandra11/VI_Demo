...
export class RechargePage {
  ...
  private async closeBlockingPopups(): Promise<void> {
    try {
      ...
      // ─── REMOVED ALL ALERT HANDLING ────────────────────────────────────────
      // No browser.getAlertText() calls = no "no such alert" logs
    } catch (_) {
      // Silently ignore
    }
  }
  ...
  // ─── Helper: Scrape Dedicated Account Table (Customer IN Profile page) ─────
  // async scrapeOtherOffersDA(): Promise<any[]> {
  //   const results: any[] = [];
  //   try {
  //     console.log("[RechargePage] Scraping Dedicated Account table...");
  //     const container = await $("#ded_acc_view"); 
  //     await container.waitForDisplayed({ timeout: 10000 });

  //     const rows = await container.$$("table tbody tr");
  //     console.log(`[RechargePage] Found ${rows.length} Dedicated Account rows`);

  //     for (let i = 0; i < rows.length; i++) {
  //       const cells = await rows[i].$$("td");
  //       if (cells.length < 7) continue;

  //       const daName    = (await cells[0].getText().catch(() => "")).trim();
  //       const daId      = (await cells[1].getText().catch(() => "")).trim();
  //       const startDate = (await cells[2].getText().catch(() => "")).trim();
  //       const expiryDate= (await cells[3].getText().catch(() => "")).trim();
  //       const daValue   = (await cells[4].getText().catch(() => "")).trim();
  //       const unit      = (await cells[5].getText().catch(() => "")).trim();
  //       const type      = (await cells[6].getText().catch(() => "")).trim();

  //       if (!daName) continue; // skip empty/header rows

  //       results.push({ daName, daId, startDate, expiryDate, daValue, unit, type });
  //     }

  //     console.log(`[RechargePage] Found ${results.length} Dedicated Account entries`);
  //     return results;
  //   } catch (error) {
  //     console.error("[RechargePage] Error scraping Dedicated Account table:", error);
  //     return [];
  //   }
  // }
  ...
  try {
    ...
    let rows = await container.$$("table tbody tr");
    if (rows.length === 0) {
      console.log("[RechargePage] No rows with table tbody tr, trying table tr...");
      rows = await container.$$("table tr");
    }
    if (rows.length === 0) {
      console.log("[RechargePage] No rows with table tr, trying tr...");
      rows = await container.$$("tr");
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Skip header rows
      if (!daName || daName === "DA Name" || daName === "DA Name*" || daName.includes("DA")) continue;
  ...
  // ─── Helper: Scrape Offer Table (Customer IN Profile page) ─────────────────
  // FIXED: same root cause as scrapeOtherOffersDA — now targets #offer,
  // the real tab pane behind #offer_tab, instead of ".bd_otherOffer".
  // async scrapeOtherOffersOfferTab(): Promise<any[]> {
  //   const results: any[] = [];
  //   try {
  //     console.log("[RechargePage] Scraping Offer table...");
  //     const container = await $("#offer");
  //     await container.waitForDisplayed({ timeout: 10000 });

  //     const rows = await container.$$("table tbody tr");
  //     console.log(`[RechargePage] Found ${rows.length} Offer rows`);

  //     for (let i = 0; i < rows.length; i++) {
  //       const cells = await rows[i].$$("td");
  //       if (cells.length < 6) continue;

  //       const offerName     = (await cells[0].getText().catch(() => "")).trim();
  //       const offerId        = (await cells[1].getText().catch(() => "")).trim();
  //       const productId       = (await cells[2].getText().catch(() => "")).trim();
  //       const startDateTime = (await cells[3].getText().catch(() => "")).trim();
  //       const endDateTime   = (await cells[4].getText().catch(() => "")).trim();
  //       const offerType      = (await cells[5].getText().catch(() => "")).trim();

  //       if (!offerName) continue;

  //       results.push({ offerName, offerId, productId, startDateTime, endDateTime, offerType });
  //     }

  //     console.log(`[RechargePage] Found ${results.length} Offer entries`);
  //     return results;
  //   } catch (error) {
  //     console.error("[RechargePage] Error scraping Offer table:", error);
  //     return [];
  //   }
  // }
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Skip header rows
      if (!offerName || offerName === "Offer Name" || offerName === "Offer Name*" || offerName.includes("Offer")) continue;
  ...
  async runPreTestVerification(msisdn: string): Promise<{
    success: boolean;
    totalUsage: { voice: any[]; data: any[]; sms: any[] };
    ...
    console.log(`[RechargePage] Running PreTest verification for MSISDN: ${msisdn}`);
    ...
    const result: {
      ...
      success: false,
      totalUsage: { voice: [], data: [], sms: [] },
      steps: [],
    };

    try {
      console.log("[RechargePage] Refreshing page before PreTest verification...");
      await browser.refresh();
      await browser.pause(3000);
      await this.closeBlockingPopups();

      // ─── STEP 1: Enter MSISDN ──────────────────────────────────────────────
      console.log("[RechargePage][PreTest] Step 1: Entering MSISDN...");
      await this.enterMSISDNswift(msisdn);
      await this.takeScreenshot("PreTest_Step1_Enter_MSISDN");
      result.steps.push({ step: "Enter MSISDN", status: "completed" });

      // ─── STEP 2: Click Search ──────────────────────────────────────────────
      console.log("[RechargePage][PreTest] Step 2: Clicking Search...");
      await this.clickSearchButtonswift();
      await this.takeScreenshot("PreTest_Step2_Click_Search");
      result.steps.push({ step: "Click Search", status: "completed" });

      // ─── STEP 3: Capture Subscriber Info ──────────────────────────────────
      console.log("[RechargePage][PreTest] Step 3: Capturing Subscriber Info...");
      const subscriberInfo = await this.captureSubscriberInfo(msisdn, 1);
      result.subscriberInfo = subscriberInfo;
      await this.takeScreenshot("PreTest_Step3_Subscriber_Info");
      result.steps.push({ step: "Subscriber Info", data: subscriberInfo });

      // ─── STEP 4: Wait for Active Offers Container ──────────────────────────
      console.log("[RechargePage][PreTest] Step 4: Waiting for Active Offers container...");
      try {
        await $(".activeOffers").waitForDisplayed({ timeout: 15000 });
        console.log("[RechargePage][PreTest]  Active Offers container loaded");
      } catch (_) {
        console.warn("[RechargePage][PreTest] ⚠️ .activeOffers container not visible");
      }

      // ─── STEP 5: Verify Recharges & Benefits ──────────────────────────────
      console.log("[RechargePage][PreTest] Step 5: Verifying Recharges & Benefits...");
      await this.verifyRechargesAndBenefits();
      await this.takeScreenshot("PreTest_Step5_Recharges_Benefits");
      result.steps.push({ step: "Verify Recharges & Benefits", status: "completed" });

      // ─── STEP 6: Click Customer IN Profile ────────────────────────────────
      console.log("[RechargePage][PreTest] Step 6: Clicking Customer IN Profile...");
      await this.clickCustomerINProfile();
      await this.takeScreenshot("PreTest_Step6_Customer_IN_Profile");
      result.steps.push({ step: "Click Customer IN Profile", status: "completed" });

      // ─── STEP 7: Verify Product Overview ──────────────────────────────────
      console.log("[RechargePage][PreTest] Step 7: Verifying Product Overview...");
      await this.verifyProductOverview();
      await this.takeScreenshot("PreTest_Step7_Product_Overview");
      result.steps.push({ step: "Verify Product Overview", status: "completed" });

      // ─── STEP 8: Capture Account Overview ──────────────────────────────────
      console.log("[RechargePage][PreTest] Step 8: Capturing Account Overview...");
      const accountOverview = await this.captureAccountOverview();
      result.accountOverview = accountOverview;
      await this.takeScreenshot("PreTest_Step8_Account_Overview");
      result.steps.push({ step: "Account Overview", data: accountOverview });

      // ─── STEP 9: Click Dedicated Account Tab ──────────────────────────────
      console.log("[RechargePage][PreTest] Step 9: Clicking Dedicated Account Tab...");
      await this.clickDedicatedAccount();
      await this.takeScreenshot("PreTest_Step9_Dedicated_Account_Tab");
      result.steps.push({ step: "Click Dedicated Account Tab", status: "completed" });

      // ─── STEP 10: Scrape Other Offers (DA Table) ──────────────────────────
      console.log("[RechargePage][PreTest] Step 10: Scraping Other Offers (DA Table)...");
      const dedicatedAccounts = await this.scrapeOtherOffersDA();
      result.dedicatedAccounts = dedicatedAccounts;
      await this.takeScreenshot("PreTest_Step10_Other_Offers_DA");
      result.steps.push({ step: "Other Offers DA", data: dedicatedAccounts });

      // ─── STEP 11: Click Offer Tab ──────────────────────────────────────────
      console.log("[RechargePage][PreTest] Step 11: Clicking Offer Tab...");
      await this.clickOfferTab();
      await this.takeScreenshot("PreTest_Step11_Offer_Tab");
      result.steps.push({ step: "Click Offer Tab", status: "completed" });

      // ─── STEP 12: Scrape Other Offers (Offer Tab Table) ──────────────────
      console.log("[RechargePage][PreTest] Step 12: Scraping Other Offers (Offer Tab)...");
      const offers = await this.scrapeOtherOffersOfferTab();
      result.offers = offers;
      await this.takeScreenshot("PreTest_Step12_Other_Offers_OfferTab");
      result.steps.push({ step: "Other Offers Offer Tab", data: offers });

      // ─── STEP 13: Click IN Profile Button ──────────────────────────────────
      console.log("[RechargePage][PreTest] Step 13: Clicking IN Profile Button...");
      await this.clickINProfileButton();
      await this.takeScreenshot("PreTest_Step13_IN_Profile_Button");
      result.steps.push({ step: "Click IN Profile Button", status: "completed" });

      // ─── STEP 14: VOICE (CALL) USAGE ──────────────────────────────────────
      console.log("[RechargePage][PreTest] 📞 Step 14: Fetching Voice/Call usage...");
      await this.clickAndWaitPreTest("#ao_consolidated_tab", 1000);

      const voiceUsage = await this.parseVoiceTab();
      console.log(`[RechargePage][PreTest] 📞 Voice/Call Usage: ${voiceUsage?.length || 0} entries`);
      await this.takeScreenshot("PreTest_Step14_TotalUsage_Voice");
      result.steps.push({ step: "Voice Usage", data: voiceUsage });

      // ─── STEP 15: DATA USAGE ──────────────────────────────────────────────
      console.log("[RechargePage][PreTest] 📊 Step 15: Fetching Data usage...");
      await this.clickAndWaitPreTest("#totalUsageTabs li:nth-child(2) a", 500);

      const dataUsage = await this.parseDataTab();
      console.log(`[RechargePage][PreTest] 📊 Data Usage: ${dataUsage?.length || 0} entries`);
      await this.takeScreenshot("PreTest_Step15_TotalUsage_Data");
      result.steps.push({ step: "Data Usage", data: dataUsage });

      // ─── STEP 16: SMS USAGE ────────────────────────────────────────────────
      console.log("[RechargePage][PreTest] ✉️ Step 16: Fetching SMS usage...");
      await this.clickAndWaitPreTest("#totalUsageTabs li:nth-child(3) a", 500);

      const smsUsage = await this.parseSMSTab();
      console.log(`[RechargePage][PreTest] ✉️ SMS Usage: ${smsUsage?.length || 0} entries`);
      await this.takeScreenshot("PreTest_Step16_TotalUsage_SMS");
      result.steps.push({ step: "SMS Usage", data: smsUsage });

      // ─── Store all usage data ──────────────────────────────────────────────
      result.totalUsage = { voice: voiceUsage, data: dataUsage, sms: smsUsage };

      // ─── FINAL SUMMARY & VALIDATION ──────────────────────────────────────
      console.log("[RechargePage][PreTest] 📋 USAGE SUMMARY:");
      console.log(`  📞 Voice/Call: ${voiceUsage?.length || 0} entries ${voiceUsage?.length > 0 ? '❌' : '✅'}`);
      console.log(`  📊 Data:       ${dataUsage?.length || 0} entries ${dataUsage?.length > 0 ? '❌' : '✅'}`);
      console.log(`  ✉️ SMS:        ${smsUsage?.length || 0} entries ${smsUsage?.length > 0 ? '❌' : '✅'}`);
      console.log(`  📁 Dedicated Accounts: ${dedicatedAccounts?.length || 0} entries ${dedicatedAccounts?.length > 0 ? '❌' : '✅'}`);
      console.log(`  🏷️ Offers:     ${offers?.length || 0} entries ${offers?.length > 0 ? '❌' : '✅'}`);

      // Check for any active usage across ALL categories
      const failedCategories: string[] = [];

      // Check Voice/Data/SMS
      if (voiceUsage?.length > 0) failedCategories.push("Voice available");
      if (dataUsage?.length > 0) failedCategories.push("Data available");
      if (smsUsage?.length > 0) failedCategories.push("SMS available");

      // ─── NEW: Check Dedicated Accounts ──────────────────────────────────
      if (dedicatedAccounts && dedicatedAccounts.length > 0) {
        // Check if there are any REAL dedicated accounts (skip header rows if any)
        const realAccounts = dedicatedAccounts.filter(da => 
          da.daName && 
          !da.daName.includes("DA Name") && 
          !da.daName.includes("DA Name*") &&
          da.daName.trim().length > 0
        );
        if (realAccounts.length > 0) {
          failedCategories.push(`Dedicated Account usage (${realAccounts.length} entries)`);
          console.log(`[RechargePage][PreTest]  ❌ Found ${realAccounts.length} Dedicated Account entries:`);
          realAccounts.forEach(da => {
            console.log(`     - ${da.daName} (${da.daId}) - Expires: ${da.expiryDate || 'N/A'}`);
          });
        }
      }

      // ─── NEW: Check Offers (skip default ones) ──────────────────────────
      const DEFAULT_OFFERS = ['1_DEFAULT', '5_4G_TAG'];
      if (offers && offers.length > 0) {
        // Filter out default offers
        const realOffers = offers.filter(offer => 
          offer.offerName && 
          !DEFAULT_OFFERS.includes(offer.offerName.trim()) &&
          !offer.offerName.includes("Offer Name") &&
          !offer.offerName.includes("Offer Name*") &&
          offer.offerName.trim().length > 0
        );
        
        if (realOffers.length > 0) {
          failedCategories.push(`Non-default offers found (${realOffers.length} entries)`);
          console.log(`[RechargePage][PreTest]  ❌ Found ${realOffers.length} non-default Offer entries:`);
          realOffers.forEach(offer => {
            console.log(`     - ${offer.offerName} (ID: ${offer.offerId})`);
          });
        }
      }
      ...
      if (failedCategories.length > 0) {
        ...
        result.reason = `PreTest Failed: ${failedCategories.join(", ")} - MSISDN Number Not Clean`;
        console.log(`[RechargePage]  ❌ PreTest FAILED for ${msisdn}: ${result.reason}`);
      } else {
        result.success = true;
        result.reason = "No active Voice/Data/SMS usage found — PreTest passed";
        console.log(`[RechargePage]  ✅ PreTest PASSED for ${msisdn}`);
      }

      // ─── Log account overview summary ──────────────────────────────────────
      if (accountOverview) {
        console.log("[RechargePage][PreTest] Account Overview Summary:");
        console.log(`  ├─ Activation Date: ${accountOverview.activationDate || 'N/A'}`);
        console.log(`  ├─ Subscriber Status: ${accountOverview.subscriberStatus || 'N/A'}`);
        console.log(`  ├─ Main Balance: ${accountOverview.mainBalance || 'N/A'}`);
        console.log(`  └─ Service Validity: ${accountOverview.serviceRemovalOn || 'N/A'}`);
      }

      console.log(`[RechargePage][PreTest] Dedicated Accounts: ${dedicatedAccounts?.length || 0} entries`);
      console.log(`[RechargePage][PreTest] Offers: ${offers?.length || 0} entries`);

      return result;

    } catch (error) {
      console.error(`[RechargePage] PreTest verification error for ${msisdn}:`, error);
      result.success = false;
      result.reason = `PreTest verification error: ${error instanceof Error ? error.message : String(error)}`;
      return result;
    }
  }

  // ─── PreTest Click Helper ──────────────────────────────────────────────────────

  private async clickAndWaitPreTest(selector: string, waitMs: number = 1000): Promise<void> {
    try {
      const element = await $(selector);
      const exists = await element.isExisting().catch(() => false);

      if (!exists) {
        console.log(`[RechargePage][PreTest] Element not found: ${selector}`);
        return;
      }

      const displayed = await element.isDisplayed().catch(() => false);
      if (!displayed) {
        console.log(`[RechargePage][PreTest] Element not displayed: ${selector}`);
        return;
      }

      await element.click();
      await browser.pause(waitMs);
      console.log(`[RechargePage][PreTest]  Clicked: ${selector}`);

    } catch (error) {
      console.log(`[RechargePage][PreTest] Error clicking ${selector}:`,
        error instanceof Error ? error.message : error);
    }
  }

  // ─── IN Test - Yes Case Testing Process ────────────────────────────────────
  ...
  try {
    ...
    // ─── STEP 7: Capture Account Overview ──────────────────────────────────
    // Wait for the account overview to load after clicking Customer IN Profile
    console.log("[RechargePage] Waiting for Account Overview to load...");
    
    // Wait for the .accountBox to be present
    try {
      await browser.waitUntil(
        async () => {
          const accountBox = await $('.accountBox');
          return await accountBox.isExisting() && await accountBox.isDisplayed();
        },
        { timeout: 15000, timeoutMsg: 'Account Overview container did not load' }
      );
      console.log("[RechargePage]  Account Overview container found");
    } catch (waitErr) {
      console.warn("[RechargePage] ⚠️ Account Overview container not found, but continuing...");
    }

    // Wait for actual data to be populated in the fields
    try {
      await browser.waitUntil(
        async () => {
          const activationDate = await $('#ActivationDateValue');
          const serviceRemoval = await $('#ServiceRemovalOnValue');
          const mainBalance = await $('#MainBalanceValue');
          const subscriberStatus = await $('#SubscriberStatusValue');
          
          const hasActivationDate = await activationDate.isExisting() && (await activationDate.getText()).trim().length > 0;
          const hasServiceRemoval = await serviceRemoval.isExisting() && (await serviceRemoval.getText()).trim().length > 0;
          const hasMainBalance = await mainBalance.isExisting() && (await mainBalance.getText()).trim().length > 0;
          const hasSubscriberStatus = await subscriberStatus.isExisting() && (await subscriberStatus.getText()).trim().length > 0;
          
          return hasActivationDate || hasServiceRemoval || hasMainBalance || hasSubscriberStatus;
        },
        { timeout: 15000, timeoutMsg: 'Account Overview fields did not populate' }
      );
      console.log("[RechargePage]  Account Overview data populated");
    } catch (dataErr) {
      console.warn("[RechargePage] ⚠️ Account Overview fields not populated, but continuing...");
    }

    const accountOverview = await this.captureAccountOverview();
    results.accountOverview = accountOverview;
    await this.takeScreenshot("IN_Step7_Account_Overview");
    results.steps.push({ step: "Account Overview", data: accountOverview });

    // ─── STEP 8: Click Dedicated Account Tab ──────────────────────────────
    await this.clickDedicatedAccount();
    await this.takeScreenshot("IN_Step8_Dedicated_Account_Tab");

    // ─── STEP 8.1: Scrape Other Offers (DA Table) ──────────────────────────
    const otherOffersDA = await this.scrapeOtherOffersDA();
    results.otherOffersDA = otherOffersDA;
    results.dedicatedAccounts = otherOffersDA;
    await this.takeScreenshot("IN_Step8_1_Other_Offers_DA");
    results.steps.push({ step: "Other Offers DA", data: otherOffersDA });

    // ─── STEP 9: Click Offer Tab ──────────────────────────────────────────
    await this.clickOfferTab();
    await this.takeScreenshot("IN_Step9_Offer_Tab");

    // ─── STEP 9.1: Scrape Other Offers (Offer Tab Table) ──────────────────
    const otherOffersOfferTab = await this.scrapeOtherOffersOfferTab();
    results.otherOffersOfferTab = otherOffersOfferTab;
    results.offers = otherOffersOfferTab;
    await this.takeScreenshot("IN_Step9_1_Other_Offers_OfferTab");
    results.steps.push({ step: "Other Offers Offer Tab", data: otherOffersOfferTab });

    // ─── STEP 10: Click IN Profile Button ──────────────────────────────────
    await this.clickINProfileButton();
    await this.takeScreenshot("IN_Step10_IN_Profile_Button");
    ...
    console.log("[RechargePage] Account Overview Summary:");
    console.log(`  ├─ Activation Date: ${accountOverview.activationDate || 'N/A'}`);
    console.log(`  ├─ Service Removal On: ${accountOverview.serviceRemovalOn || 'N/A'}`);
    console.log(`  ├─ Supervision Expires On: ${accountOverview.supervisionExpiresOn || 'N/A'}`);
    console.log(`  ├─ Main Balance: ${accountOverview.mainBalance || 'N/A'}`);
    console.log(`  ├─ Service Fee Expires On: ${accountOverview.serviceFeeExpiresOn || 'N/A'}`);
    console.log(`  ├─ Subscriber Status: ${accountOverview.subscriberStatus || 'N/A'}`);
    console.log(`  └─ Credit Clearance On: ${accountOverview.creditClearanceOn || 'N/A'}`);
...
//Swift yes case
  ...
  const smsOffers: any[] = [];
  try {
    console.log("[RechargePage] Parsing SMS tab...");
    await browser.pause(1000);

    // Wait for any SMS table to load
    await browser.waitUntil(
      async () => {
        const tables = await $$('.activeOffers table, #SMSPlan, [id*="SMS"] table, [id*="sms"] table');
        return tables.length > 0;
      },
      { timeout: 5000, timeoutMsg: 'No SMS tables found' }
    ).catch(() => {});

    // Approach 1: Look for tables containing SMS-related text
    const allTables = await $$('.activeOffers table, #SMSPlan, [id*="SMS"] table, [id*="sms"] table');
    
    for (const table of allTables) {
      const tableText = await table.getText().catch(() => "");
      
      // Skip if this table doesn't contain SMS-related content
      if (!tableText.toLowerCase().includes('sms') && 
          !tableText.toLowerCase().includes('message')) {
        continue;
      }

      const rows = await table.$$('tbody tr:not(.bdDetailTableHeader), tr:not(.bdDetailTableHeader)');
      
      for (const row of rows) {
        const cells = await row.$$('td');
        if (cells.length >= 4) {
          const offerName = (await cells[0].getText().catch(() => "")).trim();
          const balanceLeft = (await cells[1].getText().catch(() => "")).trim();
          const category = (await cells[2].getText().catch(() => "")).trim();
          const expiryDate = (await cells[3].getText().catch(() => "")).trim();
          
          // Skip header rows
          if (offerName && 
              !offerName.includes('Offer Name') && 
              !offerName.includes('Total') &&
              offerName.length > 0) {
            smsOffers.push({ 
    ...
    // Approach 2: Direct parsing based on the screenshot structure
    if (smsOffers.length === 0) {
      try {
        // Look for "Fixed SMS" entries directly
        const fixedSmsElements = await $$('div:contains("Fixed SMS"), label:contains("Fixed SMS"), span:contains("Fixed SMS")');
        for (const element of fixedSmsElements) {
          const parent = await element.parentElement();
          const table = await parent.$('table');
          if (await table.isExisting()) {
            ...
            for (const row of rows) {
              ...
              if (cells.length >= 4) {
                ...
                if (offerName && !offerName.includes('Offer Name') && offerName.length > 0) {
                  smsOffers.push({ 
                    offer_name: offerName, 
                    balance_left: balanceLeft, 
                    category: category, 
    ...
    // Approach 3: Check if there's any table with SMS category
    if (smsOffers.length === 0) {
      try {
        // Look for any table where category is "SMS"
        const allTables = await $$('table');
        for (const table of allTables) {
          const rows = await table.$$('tbody tr, tr');
          for (const row of rows) {
            const cells = await row.$$('td');
  ...
  const dataOffers: any[] = [];
  try {
    console.log("[RechargePage] Parsing Data tab...");
    await browser.pause(1000);

    // Wait for any data table to load
    await browser.waitUntil(
      async () => {
        const tables = await $$('.activeOffers table, #dataPlan, #dailyDataPlan, [id*="data"] table');
        return tables.length > 0;
      },
      { timeout: 5000, timeoutMsg: 'No data tables found' }
    ).catch(() => {});

    // Approach 1: Look for tables containing data-related text
    const allTables = await $$('.activeOffers table, #dataPlan, #dailyDataPlan, [id*="data"] table, [id*="Data"] table');
    
    for (const table of allTables) {
      const tableText = await table.getText().catch(() => "");
      
      // Skip if this table doesn't contain data-related content
      if (!tableText.toLowerCase().includes('data') && 
          !tableText.toLowerCase().includes('gb') && 
          !tableText.toLowerCase().includes('mb') &&
          !tableText.toLowerCase().includes('data plan')) {
        continue;
      }
    ...
    if (dataOffers.length === 0) {
      try {
        // Find the "Non-Daily Data packs" header and get the table after it
        const headers = await $$('div:contains("Non-Daily Data packs"), label:contains("Non-Daily Data packs"), span:contains("Non-Daily Data packs")');
        for (const header of headers) {
          const parent = await header.parentElement();
          const table = await parent.$('table');
          if (await table.isExisting()) {
            const rows = await table.$$('tbody tr, tr');
            for (const row of rows) {
