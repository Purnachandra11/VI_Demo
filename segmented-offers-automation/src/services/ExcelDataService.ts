...
//   // 2. Try match by MRP only (any circle) - WITH LOGGING
//   const mrpMatches = plans.filter(plan => plan.newMRP.trim() === normalizedMRP);
  
//   if (mrpMatches.length > 0) {
//     matchedPlan = mrpMatches[0];
//     console.log(`[ExcelDataService] ⚠️ Found MRP match (circle fallback): MRP=${matchedPlan.newMRP}, Circle=${matchedPlan.circle}`);
//     console.log(`[ExcelDataService] 📋 Available circles for MRP ${normalizedMRP}: ${mrpMatches.map(p => p.circle).join(', ')}`);
//     return matchedPlan;
//   }
...
// In ExcelDataService.ts
findMatchingPlan(mrp: string, circle?: string): RechargePlan | null {
  const normalizedMRP = mrp.trim();
  // Find by MRP only - ignore circle
  const plans = this.getRechargePlans();
  for (const plan of plans) {
    if (plan.newMRP.trim() === normalizedMRP) {
      return plan;
    }
  }
  return null;
